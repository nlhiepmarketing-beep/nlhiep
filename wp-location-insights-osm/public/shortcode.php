...
add_action( 'init', 'wpli_register_shortcode' );

/**
 * Register shortcode.
 */
function wpli_register_shortcode() {
	add_shortcode( 'wpli_location_insights', 'wpli_render_shortcode' );
}

/**
 * Enqueue frontend assets.
 */
function wpli_enqueue_frontend_assets() {
 ...
 * @return string
 */
function wpli_render_shortcode( $atts ) {
	$settings = wpli_get_settings();
	...
	if ( null === $lat || null === $lng ) {
		if ( current_user_can( 'manage_options' ) ) {
			return '<div class="wpli wpli-error">' . esc_html__( 'Thiếu toạ độ hợp lệ. Vui lòng kiểm tra shortcode hoặc vị trí.', 'wp-location-insights-osm' ) . '</div>';
		}
		return '<div class="wpli wpli-error">' . esc_html__( 'Không thể hiển thị bản đồ ở thời điểm này.', 'wp-location-insights-osm' ) . '</div>';
	}
