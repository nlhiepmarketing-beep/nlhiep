...
if ( ! class_exists( 'WPLI_REST' ) ) {
	class WPLI_REST {
		...
		public function register_routes() {
			register_rest_route(
				...
				array(
					...
					'callback'            => array( $this, 'handle_poi' ),
		...
		public function handle_poi( WP_REST_Request $request ) {
			...
			$filter = $this->get_overpass_filter( $type );
			...
			foreach ( $data['elements'] as $element ) {
				...
				$distance = $this->haversine_distance( $lat, $lng, $lat_item, $lng_item );
				...
				$items[] = array(
					...
					'address'    => $address,
					'distance_m' => $distance,
					'osm_url'    => $this->build_osm_url( $element['type'], $element['id'] ),
				);
			}
		...
		private function get_overpass_filter( $type ) {
			switch ( $type ) {
				case 'school':
		...
		private function build_osm_url( $type, $id ) {
